...
public class ServicioFactura {
	...
	public List<Factura> ListadoFacturas(Cabecera fact) {
		...
		for(int i=0;i<fact.getFacturas().size();i++) {
			...
			System.out.println("Código Cliente: "+fact.getFacturas().get(i).getCliente());
			...
			System.out.println("Días vencidos: "+fact.getFacturas().get(i).NumDiasVencidos());
		...
		System.out.println("Número de item de la cabecera: "+list_fact.size());
		return list_fact;
	}
}
